...
const SubscriptionSection: React.FC<Props> = ({
    ...
    useEffect(() => {
        const fetchPlans = async () => {
            try {
                const data =
                    await businessOwnerService.getPlans();
        ...
        setForm((prev) => ({
            ...
            subscriptions: [
...

                {
                    ...
                    // DEFAULT LIMITS
                    custom_max_tenants:
                        defaultPlan?.max_tenants ??
                        undefined,

                    custom_max_outlets:
                        defaultPlan?.max_outlets ??
                        undefined,
    ...
    >(
        ...
        setForm((prev) => {
            ...
            if (field === "plan_id") {
                ...
                if (selectedPlan) {
                    // ALWAYS SAVE PLAN ID
                    updated[index].plan_id =
                        selectedPlan.id;

                    // APPLY LIMITS
                    updated[index].custom_max_tenants =
                        selectedPlan.max_tenants ??
                        undefined;

                    updated[index].custom_max_outlets =
                        selectedPlan.max_outlets ??
                        undefined;
